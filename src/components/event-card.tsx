 ...
 */
/**
 * LE DOS, écrit une fois : la nuit, les chevrons de laiton, la marque, le nom
 * du deck. Le deck physique l'imprime tel quel, avec en plus le ruban de son
 * paquet (marché ou équipe), que l'écran n'a pas besoin de montrer.
 */
export function EventCardBack({
  ...
  /** Le paquet, pour trier une pioche face cachée : marché en ciel, équipe en laiton. */
  ruban?: "market" | "team";
  className?: string;
}) {
  return (
    <div className={`card-back-face rounded-xl border-2 border-amber-400/40 ${ruban ? "relative" : ""} ${className}`}>
      <div className={`flex h-full flex-col items-center justify-center gap-2 rounded-lg border border-amber-400/20 ${ruban ? "pb-6" : ""}`}>
        ...
        {deckName ? (
          <span className="text-xs uppercase tracking-[0.2em] text-amber-400/50">{deckName}</span>
        ) : null}
      </div>
      {ruban ? (
        <span
          className={`absolute inset-x-2 bottom-3 whitespace-nowrap rounded px-1 py-1 text-center text-xs font-bold uppercase tracking-normal text-slate-950 ${
            ruban === "market" ? "bg-sky-300" : "bg-amber-400"
          }`}
        >
          {ruban === "market" ? "Marché · toute la classe" : "Équipe · tirage ciblé"}
        </span>
      ) : null}
    </div>
  );
...
/**
 * LA FACE, écrite une fois : l'écran la retourne, le papier l'imprime. C'est
 * ce partage qui garantit qu'une carte tirée en classe est la même que celle
 * que l'élève voit dans l'arène.
 */
export function EventCardFace({
  code,
  announced = false,
  targetLabel,
  highlight = false,
  ...
  return (
    <div className="card-flip-scene" style={{ animationDelay: `${delayMs}ms` }}>
      <div className="card-flip" style={{ animationDelay: `${delayMs}ms` }}>
        {/* dos de la carte */}
        <EventCardBack deckName={position?.deck} className="card-face card-back" />
        {/* face de la carte */}
        <EventCardFace code={code} announced={announced} targetLabel={targetLabel} highlight={highlight} />
      </div>
    </div>
  );
}
