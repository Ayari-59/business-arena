...
} from "../../engine/market/communication";
...
export function referencesDuDossier(
  definition: ScenarioDefinition,
  tours: number,
): ReferenceDossier[] {
  const config = definition.scenario;
  const etat = definition.company(
    "dossier",
...
function tourFort(definition: ScenarioDefinition, tours: number): number {
  ...
  for (let t = 1; t <= tours; t++) {
    const demande = gamme.reduce(
      (s, p) =>
        s +
        p.market.segments.reduce(
          (u, seg) =>
            u +
            seg.size *
              (seg.seasonality?.[t - 1] ?? p.market.seasonality[t - 1] ?? 1),
          0,
        ),
      0,
    );
 ...
 */
function axesPourClientele(segment: SegmentConfig): {
  portent: string[];
  desservent: string[];
} {
  const portent: string[] = [];
  const desservent: string[] = [];
  const nom = (a: CommunicationAxis) =>
    COMMUNICATION_AXIS_LABELS[a].label
      .replace(/^(Le |La |L')/, "")
      .toLowerCase();
  for (const axe of COMMUNICATION_AXES) {
    if (axe === "innovation") continue;
    const a = axisAffinity(axe, segment, {
      price: segment.refPrice,
      techLevel: 0,
...
export function dossiersDeService(
  ...
  const config = options.sansRd
    ? withoutRd(definition.scenario)
    : definition.scenario;
  ...
  const clienteles = toGamme(config).flatMap((p) =>
    p.market.segments.map((s) => ({
      produit: p.name,
      segment: s,
      saison: p.market.seasonality,
    })),
  );
  ...
  const aDevelopper = config.rd
    ? toGamme(config).filter((p) => p.development)
    : [];
  ...
  const approvisionnement: DossierService = {
    ...
    lignes: [
      {
        libelle: `${v.capacityLabel}`,
        valeur: `${entier(etat.machineCapacity)} ${v.perRoundLabel}`,
      },
      {
        libelle: `${v.leftoverLabel} à l'ouverture`,
        valeur: `${entier(gamme.reduce((s, g) => s + g.stockOuverture, 0))} ${v.units}`,
      },
      {
        libelle: "Délai de règlement des fournisseurs",
        valeur: jours(config.finance.supplierPaymentDelayDays),
      },
      ...aDevelopper.map((p) => ({
      ...
      ...toGamme(config).flatMap((p) => {
        ...
        return catalogue.map((s) => {
          const ratio = reference
            ? s.costMultiplier / reference.costMultiplier
            : s.costMultiplier;
    ...
    tableau: {
      entetes: [
        multi ? "Référence" : "Produit",
        v.materialLabel,
        v.otherVariableLabel,
        "Coût variable",
        `${v.leftoverLabel} à l'ouverture`,
      ],
      lignes: gamme.map((g) => [
        g.nom,
        euro(g.coutAchat),
        euro(g.fraisVariables),
        euro(g.coutVariable),
        `${entier(g.stockOuverture)} ${v.units}`,
      ]),
    },
    questions: [
  ...
  const commercial: DossierService = {
    ...
    lignes: [
      {
        libelle: "Budget marketing de référence par tour",
        valeur: euro(0.5 * config.marketing.scale),
      },
      ...(marque
        ? [
            {
      ...
      ...(config.orderOffers ?? []).slice(0, 3).map((o) => ({
        ...
        valeur: `${entier(o.units)} ${v.units} à ${euro(o.price)}, règlement ${jours(o.paymentDelayDays)}${
          multi
            ? ` · sur la référence ${gamme[offerProductIndex(gamme, o)]!.nom}`
            : ""
        }`,
      })),
    ],
  ...
  const rh: DossierService = {
    ...
    lignes: [
      ...
      {
        libelle: `${temps.Pluriel} disponibles par tour`,
        valeur: `${entier(heures)} ${temps.abrege} (${etat.hoursPerEmployee} ${temps.abrege} par personne)`,
      },
      ...(config.hr
        ? [
            {
              libelle: "Salaire chargé par personne et par tour",
              valeur: euro(config.hr.salaryPerEmployeePerRound),
            },
            {
              libelle: "Coût d'une embauche",
              valeur: euro(config.hr.hiringCost),
            },
            {
              libelle: "Coût d'un licenciement",
              valeur: euro(config.hr.firingCost),
            },
            {
              libelle: "Budget de formation de référence",
              valeur: euro(config.hr.trainingScale),
            },
            {
              libelle: "Embauches possibles par tour",
              valeur: `${config.hr.maxHiresPerRound} (effectif maximal ${config.hr.maxHeadcount})`,
            },
          ]
        : []),
    ],
    tableau: {
      entetes: [
        multi ? "Référence" : "Produit",
        `${temps.Pluriel} par unité`,
        `Capacité de l'équipe (${v.perRoundLabel})`,
      ],
  ...
  const financier: DossierService = {
    ...
    mission:
      "Prévoir la trésorerie du tour avant de valider : ce qui entre, ce qui sort, et quand. Un résultat positif ne garantit pas une caisse positive.",
    lignes: [
      { libelle: "Trésorerie à l'ouverture", valeur: euro(etat.finance.cash) },
      {
        libelle: "Créances clients à encaisser",
        valeur: euro(etat.finance.receivables),
      },
      {
        libelle: "Dettes fournisseurs à régler",
        valeur: euro(etat.finance.payables),
      },
      {
        libelle: `${v.leftoverLabel} (valeur au bilan)`,
        valeur: euro(etat.finance.inventoryValue),
      },
      {
        libelle: "Immobilisations nettes",
        valeur: euro(etat.finance.fixedAssetsNet),
      },
      { libelle: "Capitaux propres", valeur: euro(etat.finance.equity) },
      {
        libelle: "Emprunt restant dû",
        valeur: `${euro(restant)} · échéance ${euro(echeance)} par tour`,
      },
      {
        libelle: "Charges de structure décaissées par tour",
        valeur: euro(config.fixedCostsPerRound),
      },
      {
        libelle: "Amortissements par tour",
        valeur: euro(config.finance.depreciationPerRound),
      },
      {
        libelle: "Taux d'emprunt · taux de découvert",
        valeur: `${pct(config.finance.loanAnnualRate)} · ${pct(config.finance.overdraftAnnualRate)} par an`,
      },
      {
        libelle: "Découvert autorisé",
        valeur: euro(config.finance.overdraftLimit),
      },
      {
        libelle: "Impôt sur les bénéfices",
        valeur: pct(config.finance.taxRate),
      },
      ...aDevelopper.map((p) => ({
