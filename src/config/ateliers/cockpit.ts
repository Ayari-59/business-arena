...
export function cockpitSpec(source: CockpitSource): ClasseurSpec {
  ...
  const etat: CompanyState = definition.company(
    "cockpit",
    definition.playerTeamName,
    "human",
  );
  ...
  const diviseur = source.config
    ? Math.max(1, source.concurrents)
    : Math.min(Math.max(1, source.concurrents), 3);
  const hist = source.historique;
  const partDifferee = Math.min(
    1,
    config.finance.supplierPaymentDelayDays / config.roundDays,
  );
  ...
  const poser = (
    ...
    P.push([
      t(libelle),
      n(valeur, format, "saisie"),
      note ? t(note, "note") : vide(),
    ]);
  ...
  P.push([
    t(
      hist
        ? `Équipe ${hist.equipe}`
        : "Feuille de paramètres : ce que le jeu applique. Modifiez-la seulement si l'enseignant a changé les règles.",
      "note",
    ),
  ]);
  P.push([]);
  ...
  poser(
    "capacite",
    `${v.capacityLabel} (${v.perRoundLabel})`,
    Math.round(etat.machineCapacity),
    "unites",
  );
  poser(
    "heures",
    `${temps.Pluriel} de main-d'œuvre disponibles par tour`,
    Math.round(etat.headcount * etat.hoursPerEmployee * etat.productivity),
    "unites",
    `${etat.headcount} personnes × ${etat.hoursPerEmployee} ${temps.abrege}`,
  );
  poser(
    "fixes",
    "Charges de structure décaissées par tour",
    config.fixedCostsPerRound,
    "euro",
  );
  poser(
    "amortissements",
    "Amortissements par tour",
    config.finance.depreciationPerRound,
    "euro",
  );
  poser(
    "marketingRef",
    "Budget marketing de référence par tour",
    Math.round(0.5 * config.marketing.scale),
    "euro",
  );
  poser(
    "qualiteRef",
    "Budget qualité de référence par tour",
    Math.round(0.5 * config.production.qualityScale),
    "euro",
  );
  poser(
    "maintenanceRef",
    "Budget maintenance de référence par tour",
    Math.round(config.production.maintenanceReference),
    "euro",
  );
  ...
  poser(
    "partCredit",
    "Part du chiffre d'affaires encaissée au tour suivant",
    Math.round(credit * 100) / 100,
    "pct",
    "clientèles qui règlent à délai, pondérées par leur poids",
  );
  poser(
    "partDifferee",
    "Part des achats payée au tour suivant",
    Math.round(partDifferee * 100) / 100,
    "pct",
    `délai fournisseur ${config.finance.supplierPaymentDelayDays} j sur un tour de ${config.roundDays} j`,
  );
  P.push([]);
  ...
  poser(
    "caisse",
    "Trésorerie",
    Math.round(ouverture?.caisse ?? etat.finance.cash),
    "euro",
  );
  poser(
    "creances",
    "Créances clients à encaisser",
    Math.round(ouverture?.creances ?? etat.finance.receivables),
    "euro",
  );
  poser(
    "dettes",
    "Dettes fournisseurs à régler",
    Math.round(ouverture?.dettesFournisseurs ?? etat.finance.payables),
    "euro",
  );
  P.push([]);
  ...
  for (const p of gamme) {
    const stock =
      ouverture?.stocks[p.code] ??
      etat.finishedGoodsByProduct?.[p.code]?.quantity ??
      (gamme.length === 1 ? etat.finishedGoods.quantity : 0);
    P.push([
      t(p.name),
  ...
  if (config.rd) {
    ...
    for (const p of aDevelopper) {
      ...
      poser(
        `devCout_${p.code}`,
        `Coût de développement · ${p.name}`,
        Math.round(dev.cost),
        "euro",
      );
      poser(
        `devTour_${p.code}`,
        `Vendable au plus tôt au tour · ${p.name}`,
        dev.availableFromRound ?? 1,
        "unites",
      );
      poser(
        `devEngage_${p.code}`,
        `Déjà engagé avant le tour ${tours[0]} · ${p.name}`,
        Math.round(developpement[p.code]?.engage ?? 0),
        "euro",
      );
    }
    if (aDevelopper.length === 0)
      P.push([
        t(
          "Toutes les références sont lancées : la R&D ne sert plus qu'au niveau technique.",
          "note",
        ),
      ]);
  }
  ...
  L.push([
    t("", "entete"),
    ...tours.map((tour) => t(`Tour ${tour}`, "entete")),
  ]);
  ...
  for (const p of gamme) {
    ...
    poserLigne(
      ...
      tours.map((tour) => {
        ...
        if (dernierProduit && dernier) {
          ...
          return n(
            Math.round(saisonVue > 0 ? (vu * saison) / saisonVue : vu),
            "unites",
            "saisie",
          );
        }
    ...
    poserLigne(
      ...
      tours.map(() =>
        n(dernierProduit?.prix ?? prixUsuel(p), "euro", "saisie"),
      ),
    );
    ...
    const siVendable = (i: number, formule: string) =>
      enDeveloppement ? `(${formule})*${colonne(i)}${rangVendable}` : formule;
    poserLigne(
      "plan",
      ...
      tours.map((_, i) =>
        f(
          siVendable(
            i,
            `MAX(0,${colonne(i)}${rangDemande}-${colonne(i)}${rangStockDebut})`,
          ),
          "unites",
          "saisie",
        ),
    ...
    poserLigne(
      ...
      tours.map((_, i) =>
        i === 0
          ? f(gammeRef(p.code, "G"), "unites")
          : f(`${colonne(i - 1)}${rangDemande + 6}`, "unites"),
      ),
    );
    poserLigne(
      "disponible",
      ...
      tours.map((_, i) =>
        f(`${colonne(i)}${rangStockDebut}+${colonne(i)}${rangPlan}`, "unites"),
      ),
    );
    ...
    poserLigne(
      ...
      tours.map((_, i) =>
        f(
          siVendable(
            i,
            `MIN(${colonne(i)}${rangDisponible},${colonne(i)}${rangDemande})`,
          ),
          "unites",
        ),
      ),
    ...
    poserLigne(
      ...
      tours.map((_, i) =>
        f(
          `${colonne(i)}${rangDisponible}-${colonne(i)}${rangVentes}`,
          "unites",
        ),
      ),
    );
    poserLigne(
      ...
      "Demande non servie",
      tours.map((_, i) =>
        f(`${colonne(i)}${rangDemande}-${colonne(i)}${rangVentes}`, "unites"),
      ),
    );
    poserLigne(
      "ca",
      ...
      tours.map((_, i) =>
        f(`${colonne(i)}${rangPrix}*${colonne(i)}${rangVentes}`, "euro"),
      ),
    );
    poserLigne(
      "achats",
      ...
      tours.map((_, i) =>
        f(`${gammeRef(p.code, "C")}*${colonne(i)}${rangPlan}`, "euro"),
      ),
    );
    poserLigne(
      "cv",
      ...
      tours.map((_, i) =>
        f(`${gammeRef(p.code, "E")}*${colonne(i)}${rangVentes}`, "euro"),
      ),
    );
    poserLigne(
      "marge",
      ...
      tours.map((_, i) =>
        f(
          `${colonne(i)}${rangDemande + 8}-${colonne(i)}${rangDemande + 10}`,
          "euro",
        ),
      ),
    );
    if (config.rd) {
      ...
      poserLigne(
        "rd",
        dev
          ? "Recherche et développement (à saisir — préremplie : de quoi lancer la référence)"
          : "Recherche et développement (à saisir)",
        tours.map((_, i) =>
          dev && i === 0
            ? f(`MAX(0,${cout}-${engage})`, "euro", "saisie")
            : n(0, "euro", "saisie"),
        ),
      );
      if (dev) {
        if (L.length !== rangRd)
          throw new Error(
            `cockpit : la ligne R&D de ${p.code} n'est pas au rang attendu`,
          );
        poserLigne(
          "rdCumul",
          ...
          tours.map((_, i) =>
            i === 0
              ? f(engage!, "euro")
              : f(
                  `${colonne(i - 1)}${rangRdCumul}+${colonne(i - 1)}${rangRd}`,
                  "euro",
                ),
          ),
        );
        poserLigne(
          ...
          tours.map((tour, i) =>
            f(
              `IF(${colonne(i)}${rangRdCumul}>=${cout},IF(${tour}>=${tourMin},1,0),0)`,
              "unites",
            ),
          ),
        );
  ...
  poserTotal(
    "plan",
    `Total ${v.productionPlanLabel.toLowerCase()}`,
    tours.map((_, i) => f(somme("plan", i), "unites")),
  );
  poserTotal(
    "capacite",
    v.capacityLabel,
    tours.map(() => f(param.capacite!, "unites")),
  );
  poserTotal(
    "depassement",
    ...
    tours.map((_, i) =>
      f(
        `MAX(0,${colonne(i)}${totaux.plan}-${colonne(i)}${totaux.capacite})`,
        "unites",
      ),
    ),
  );
    ...
    tours.map((_, i) =>
      f(
        gamme
          .map(
            (p) =>
              `${gammeRef(p.code, "F")}*${colonne(i)}${ligneL[p.code]!.plan}`,
          )
          .join("+"),
        "unites",
      ),
  ...
  poserTotal(
    "heuresDispo",
    `${temps.Pluriel} disponibles`,
    tours.map(() => f(param.heures!, "unites")),
  );
  poserTotal(
    "ventes",
    "Total ventes prévues",
    tours.map((_, i) => f(somme("ventes", i), "unites")),
  );
  poserTotal(
    "manque",
    "Total demande non servie",
    tours.map((_, i) => f(somme("manque", i), "unites")),
  );
  poserTotal(
    "ca",
    "Chiffre d'affaires prévu, toutes références",
    tours.map((_, i) => f(somme("ca", i), "euro")),
  );
  poserTotal(
    "achats",
    `Total ${v.materialLabel.toLowerCase()}`,
    tours.map((_, i) => f(somme("achats", i), "euro")),
  );
  poserTotal(
    "cv",
    "Coût variable des ventes, toutes références",
    tours.map((_, i) => f(somme("cv", i), "euro")),
  );
  poserTotal(
    "marge",
    "Marge sur coût variable, toutes références",
    tours.map((_, i) => f(somme("marge", i), "euro")),
  );
  if (config.rd) {
    poserTotal(
      "rd",
      "Recherche et développement, toutes références",
      tours.map((_, i) => f(somme("rd", i), "euro")),
    );
  }
  ...
  const logi = (cle: string, i: number) =>
    ref(NOM_LOGISTIQUE, `${colonne(i)}${totaux[cle]}`);
  ...
  poserR(
    "ca",
    "Chiffre d'affaires",
    tours.map((_, i) => f(logi("ca", i), "euro")),
  );
  poserR(
    "cv",
    "Coût variable des ventes",
    tours.map((_, i) => f(logi("cv", i), "euro")),
  );
  poserR(
    "mcv",
    "Marge sur coût variable",
    tours.map((_, i) =>
      f(`${colonne(i)}${ligneR.ca}-${colonne(i)}${ligneR.cv}`, "euro"),
    ),
  );
  poserR(
    "marketing",
    "Budget marketing (à saisir)",
    tours.map(() => f(param.marketingRef!, "euro", "saisie")),
  );
  if (param.marqueRef) {
    poserR(
      "marque",
      "Budget de marque (à saisir)",
      tours.map(() => f(param.marqueRef!, "euro", "saisie")),
    );
  }
  poserR(
    "qualite",
    "Budget qualité (à saisir)",
    tours.map(() => f(param.qualiteRef!, "euro", "saisie")),
  );
  poserR(
    "maintenance",
    "Budget maintenance (à saisir)",
    tours.map(() => f(param.maintenanceRef!, "euro", "saisie")),
  );
  if (config.rd) {
    poserR(
      "rd",
      "Recherche et développement",
      tours.map((_, i) => f(logi("rd", i), "euro")),
    );
  }
  poserR(
    "fixes",
    "Charges de structure",
    tours.map(() => f(param.fixes!, "euro")),
  );
  poserR(
    "amortissements",
    "Amortissements",
    tours.map(() => f(param.amortissements!, "euro")),
  );
  ...
  poserR(
    ...
    tours.map((_, i) =>
      f(
        [
          `${colonne(i)}${ligneR.mcv}`,
          ...chargesDecaissees.map((c) => `${colonne(i)}${ligneR[c]}`),
          `${colonne(i)}${ligneR.amortissements}`,
        ].join("-"),
        "euro",
      ),
    ),
  ...
  poserR(
    ...
    tours.map((_, i) =>
      f(`MAX(0,${colonne(i)}${ligneR.re})*${param.is}`, "euro"),
    ),
  );
  poserR(
    "rn",
    "Résultat net prévu",
    tours.map((_, i) =>
      f(`${colonne(i)}${ligneR.re}-${colonne(i)}${ligneR.impot}`, "euro"),
    ),
  );
  poserR(
    "cumul",
    ...
    tours.map((_, i) =>
      i === 0
        ? f(`${colonne(0)}${ligneR.rn}`, "euro")
        : f(
            `${colonne(i - 1)}${ligneR.rn! + 1}+${colonne(i)}${ligneR.rn}`,
            "euro",
          ),
    ),
  );
  ...
  poserR(
    ...
    tours.map((_, i) =>
      i === 0
        ? f(
            `${colonne(0)}${ligneR.ca}*(1-${param.partCredit})+${param.creances}`,
            "euro",
          )
        : f(
            `${colonne(i)}${ligneR.ca}*(1-${param.partCredit})+${colonne(i - 1)}${ligneR.ca}*${param.partCredit}`,
            "euro",
          ),
    ),
  );
  poserR(
    ...
    tours.map((_, i) =>
      i === 0
        ? f(
            `${logi("achats", 0)}*(1-${param.partDifferee})+${param.dettes}`,
            "euro",
          )
        : f(
            `${logi("achats", i)}*(1-${param.partDifferee})+${logi("achats", i - 1)}*${param.partDifferee}`,
            "euro",
          ),
    ),
  );
  poserR(
    ...
    tours.map((_, i) =>
      f(
        [...chargesDecaissees, "impot"]
          .map((c) => `${colonne(i)}${ligneR[c]}`)
          .join("+"),
        "euro",
      ),
    ),
  ...
  poserR(
    "emprunt",
    "Échéance d'emprunt",
    tours.map(() => f(param.echeance!, "euro")),
  );
  poserR(
    "fin",
  ...
  poserR(
    ...
    tours.map((_, i) =>
      f(`MAX(0,-${colonne(i)}${ligneR.fin}-${param.decouvert})`, "euro"),
    ),
  );
  ...
  const feuilles: FeuilleSpec[] = [
    {
      nom: NOM_PARAMETRES,
      largeurs: [52, 14, 16, 16, 14, 14, 18, ...tours.map(() => 10)],
      lignes: P,
    },
  ...
  if (hist && hist.tours.length > 0) {
    ...
    H.push([
      t(
        "Ce que les tours joués ont donné, tel que le jeu l'a calculé.",
        "note",
      ),
    ]);
    H.push([]);
    H.push([
      t("", "entete"),
      ...joues.map((tour) => t(`Tour ${tour}`, "entete")),
    ]);
    for (const p of gamme) {
      H.push([t(p.name.toUpperCase(), "section")]);
      const serie = (
        cle: keyof HistoriqueEquipe["tours"][number]["produits"][number],
        format: Format,
      ) =>
        hist.tours.map((x) => {
      ...
      H.push([
        t(`${v.leftoverLabel} en fin de tour`),
        ...serie("stockFin", "unites"),
      ]);
      H.push([t("Chiffre d'affaires"), ...serie("chiffreAffaires", "euro")]);
      if (config.rd)
        H.push([t("Recherche et développement"), ...serie("rdEngage", "euro")]);
      H.push([]);
    }
    H.push([t("L'ENTREPRISE", "section")]);
    H.push([
      t("Chiffre d'affaires"),
      ...hist.tours.map((x) => n(x.chiffreAffaires, "euro")),
    ]);
    H.push([
      t("Résultat net"),
      ...hist.tours.map((x) => n(x.resultatNet, "euro")),
    ]);
    H.push([
      t("Trésorerie nette"),
      ...hist.tours.map((x) => n(x.tresorerieNette, "euro")),
    ]);
    feuilles.push({
      nom: NOM_HISTORIQUE,
      largeurs: [40, ...joues.map(() => 14)],
      lignes: H,
    });
  }

  return {
    fichier: `cockpit-${sansAccents(definition.playerTeamName)}${hist ? `-${sansAccents(hist.equipe)}` : ""}`,
    feuilles,
  };
}
  ...
  for (const fe of classeur.feuilles) {
    fe.lignes.forEach((ligne, i) => {
      for (const cellule of ligne) {
        ...
        const cibles: string[] = [];
        for (const m of cellule.f.matchAll(
          /(?:'([^']+)'!)?\$?([A-Z]+)\$?(\d+)/g,
        )) {
