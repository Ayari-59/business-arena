...
const BAR_ACCENT: Record<string, string> = {
  industrie: "bg-blue-400",
  commerce: "bg-pink-400",
  ecommerce: "bg-violet-400",
  hotellerie: "bg-amber-400",
  restauration: "bg-orange-400",
  services: "bg-cyan-400",
  abonnement: "bg-lime-400",
  batiment: "bg-yellow-400",
...
export default async function ArenaPage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  function trend(current: number, key: "revenue" | "netIncome" | "netTreasury"): { direction: "up" | "down" | "flat"; label: string } | undefined {
    if (view!.history.length < 2) return undefined;
    const prev = view!.history.at(-2)![key];
    ...
    <main id="main" className="mx-auto max-w-5xl px-6 pt-6 pb-12">
        ...
        <section className="mt-6 rounded-xl border border-amber-400/30 bg-slate-900 p-6 text-center">
          <h2 className="text-xl font-bold text-amber-300">
            {view.ranking.find((row) => row.isPlayer)?.rank === 1
              ...
              : "Partie terminée. Analysez votre trajectoire dans l'onglet Résultats."}
          </h2>
      ...
      <div className="mt-6">
        <ArenaLayout
          ...
          accentClass={BAR_ACCENT[view.sector] ?? "bg-amber-400"}
        >
          {{
            /* ═══ SITUATION TAB ═══ */
            situation: !finished ? (
            ...
            /* ═══ MÉMOIRE TAB ═══ */
            memoire: r && situations.debriefedByRound.length > 0 ? (
              <section className="space-y-4">
                <p className="text-sm text-slate-400">
                  Les situations des tours précédents et ce qu&apos;il fallait en retenir.
                </p>
